...
def login_doctor(request):
    if request.method == 'POST':
        email = request.POST.get('email')
        password = request.POST.get('password')
        user_model = models.Doctor
        jwt_token = jwt_adapter.get_jwt_by_email_password(email, password, user_model)
        if jwt_token is None:
            context = {'message': 'Invalid credentials'}
            return render(request, 'login_doc.html', context)
        else:
            # Redirect to the index1 view
            # return HttpResponseRedirect(reverse('index1'))
            context = {'jwt_token': jwt_token}
            # print('jwt_toekn: '+jwt_token)
            current_login_name.append(email)
            return render(request, 'index1.html', context)
    else:
        # Render the login_doc.html template for GET requests
